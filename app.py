...
st.set_page_config(page_title="CDL One-Click Analyst v12", layout="wide")
...
st.markdown("""
<div class="hero">
  <div class="hero-title">CDL <span class="accent">One‑Click Analyst v12</span></div>
  <div class="hero-sub">
    Built for match day: refresh live/current matches, select a game, then analyse it. 
...
Do not invent matches. Include live/in-play matches if any are currently being played.
"""
...
""", unsafe_allow_html=True)
        ...
        st.info("No best players returned.")
...
if saved:
    st.success(f"Saved analysis found from {saved.get('saved_at')}. Viewing it uses 0 extra API/Cito calls.")
else:
    st.warning("No saved analysis for this exact match/maps setup yet.")
