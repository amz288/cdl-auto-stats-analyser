...
with tab_match:
    ...
    if not st.session_state.matches:
        ...
        if go:
            try:
                with st.spinner("Confirming current rosters…"):
                    live_rosters, _ = ai_confirm_rosters(openai_key, model, match["team_a"], match["team_b"])
                ...
                with st.spinner("Running full analysis…"):
                    parsed = extract_json(openai_search(openai_key, model, ai_full_analysis(openai_key, model, match, new_maps, stats_df, live_rosters)))
                st.session_state.saved[key] = {"saved_at": now(), "match": match, "maps": new_maps,
                                               "parsed": parsed, "polymarket": pm,
                                               "live_rosters": live_rosters,
