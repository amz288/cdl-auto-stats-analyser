...
# Verified team → Cito slug map (from the real schedule response)
TEAM_TO_SLUG = {
    ...
    "Los Angeles Guerrillas M8": "la-guerrillas-m8",
...
def ai_confirm_rosters(key, model, team_a, team_b):
    """Live-confirm the CURRENT active 4-man roster for both teams via web search.
    Returns {team: [players]} using the live result, falling back to baked-in only if AI fails."""
    today = datetime.now().strftime("%Y-%m-%d")
    baked_a = ", ".join(get_roster_for(team_a)[0])
    baked_b = ", ".join(get_roster_for(team_b)[0])
    prompt = f"""Today is {today}. Use web search to find the CURRENT active 4-player starting rosters
for these two CDL 2026 (Black Ops 7) teams, RIGHT NOW. Roster moves happen weekly, so do not rely on
old data — check the most recent info on breakingpoint.gg, cod-esports.fandom.com, callofdutyleague.com,
esports news and X/Twitter. Account for any drops, benchings, subs or new signings.

Teams: {team_a}, {team_b}
For reference, possibly-outdated rosters were:
  {team_a}: {baked_a}
  {team_b}: {baked_b}
Verify these are still correct and replace any player who has been dropped/benched/subbed.

Return ONLY JSON (no prose, no fences):
{{"rosters":{{"{team_a}":["p1","p2","p3","p4"],"{team_b}":["p1","p2","p3","p4"]}}}}
Each team must have exactly its 4 current starters. Use the players' competitive handles."""
    try:
        raw = openai_search(key, model, prompt)
        parsed = extract_json(raw) or {}
        rosters = parsed.get("rosters", {}) if isinstance(parsed, dict) else {}
        out = {}
        for team in (team_a, team_b):
            players = rosters.get(team) or rosters.get(team.lower()) or []
            players = [safe(p) for p in players if safe(p)][:4]
            out[team] = players if len(players) >= 3 else get_roster_for(team)[0]
        return out, raw
    except Exception:
        return {team_a: get_roster_for(team_a)[0], team_b: get_roster_for(team_b)[0]}, ""

def ai_player_stats(key, model, team_a, team_b, live_rosters=None):
    """Get per-mode stats for all 8 players from public sites via OpenAI web search."""
    lr = live_rosters or {}
    a_players = ", ".join(lr.get(team_a) or get_roster_for(team_a)[0])
    b_players = ", ".join(lr.get(team_b) or get_roster_for(team_b)[0])
...
def ai_full_analysis(key, model, match, maps, stats_df, live_rosters=None):
    lr = live_rosters or {}
    a_players = ", ".join(lr.get(match["team_a"]) or get_roster_for(match["team_a"])[0])
    b_players = ", ".join(lr.get(match["team_b"]) or get_roster_for(match["team_b"])[0])
...
CURRENT CONFIRMED ROSTERS (only these players exist for these teams — use exactly these):
...
Do: use the stats as the base, then web-search current form, subs and roster news. Then for EACH of the 5
selected maps, give projected kills for ALL 8 starting players (4 per team) on that map's mode, ranked
high to low, AND say which team is favoured to win that map with a probability. Also give an overall
series pick with win probabilities for each team. Only name the current roster players listed above.
...
{{
  ...
  "maps":[
    {{
      "map":1,"mode":"","map_name":"","picked_by":"",
      "map_winner":"","map_winner_prob":0.0,"map_reason":"",
      "players":[{{"player":"","team":"","projected_kills":0.0,"note":""}}]
    }}
  ],
...
In "maps", include all 5 maps, and each map's "players" array MUST contain all 8 starters ranked by projected_kills descending."""
...
# ============================================================
# CITO API (schedule + rosters primary; stats as grounding)
# ============================================================

def cito_key():
    return get_secret("CITO_API_KEY")

@st.cache_data(ttl=120, show_spinner=False)
def cito_get(path):
    """GET a Cito endpoint. Returns parsed JSON dict or {'_error': ...}."""
    key = cito_key()
    if not key:
        return {"_error": "no key"}
    try:
        r = requests.get(CITO_BASE + path, headers={"x-api-key": key, "accept": "application/json"}, timeout=20)
        try:
            j = r.json()
        except Exception:
            return {"_error": f"non-JSON ({r.status_code})"}
        if not r.ok or (isinstance(j, dict) and j.get("success") is False):
            err = (j.get("error") or {}).get("code") if isinstance(j, dict) else r.status_code
            return {"_error": str(err or r.status_code)}
        return j
    except Exception as e:
        return {"_error": str(e)}
...
@st.cache_data(ttl=120, show_spinner=False)
def cito_schedule():
    """Match list from Cito CDL schedule + live. Returns (rows, error_or_None)."""
    rows, err = [], None
    seen = set()
    for path in ["/cdl/live", "/cdl/schedule"]:
        j = cito_get(path)
        if isinstance(j, dict) and j.get("_error"):
            err = j["_error"]; continue
        for m in (j.get("data") or []):
            t1, t2 = m.get("team1") or {}, m.get("team2") or {}
            a, b = safe(t1.get("name")), safe(t2.get("name"))
            if not (a and b):
                continue
            mid = safe(m.get("matchId"))
            keyt = (mid or a + b + safe(m.get("startsAt")))
            if keyt in seen:
                continue
            seen.add(keyt)
            status = "live" if safe(m.get("status")) == "live" else "upcoming"
            rows.append({
                "match_id": mid,
                "team_a": norm_team(a), "team_b": norm_team(b),
                "team_a_slug": safe(t1.get("slug")), "team_b_slug": safe(t2.get("slug")),
                "start_time": safe(m.get("startsAt"))[:16].replace("T", " "),
                "event": safe((m.get("tournament") or {}).get("name")) or safe(m.get("round")) or "CDL",
                "status": status,
                "score": safe((m.get("score") or {}).get("formatted")),
                "source": "Cito",
            })
    # live first
    rows.sort(key=lambda m: 0 if m["status"] == "live" else 1)
    return rows, (err if not rows else None)

@st.cache_data(ttl=300, show_spinner=False)
def cito_roster(team_name):
    """Authoritative current roster from Cito. Returns (list_of_igns, error_or_None)."""
    slug = TEAM_TO_SLUG.get(team_name)
    if not slug:
        return [], "no slug"
    j = cito_get(f"/teams/{slug}/roster")
    if isinstance(j, dict) and j.get("_error"):
        return [], j["_error"]
    cr = ((j.get("data") or {}).get("currentRoster")) or {}
    players = []
    for p in cr.get("players", []):
        ign = safe((p.get("player") or {}).get("currentIgn")) or safe(p.get("playerName"))
        if ign:
            players.append(ign)
    return players[:4], (None if players else "empty")

@st.cache_data(ttl=300, show_spinner=False)
def cito_player_grounding(ign):
    """Pull whatever real stats Cito has for a player, as grounding for the AI.
    Returns a compact dict; values may be 0 if Cito hasn't synced rate stats."""
    j = cito_get(f"/players/{ign.lower()}/stats")
    if isinstance(j, dict) and j.get("_error"):
        return None
    css = ((j.get("data") or {}).get("currentSeasonStats")) or {}
    o = css.get("overall") or {}
    hp = css.get("hardpoint") or {}
    snd = css.get("searchAndDestroy") or {}
    ctrl = css.get("control") or {}
    g = {
        "ign": ign,
        "kd": o.get("kdRatio") or 0,
        "kills": o.get("kills") or 0,
        "deaths": o.get("deaths") or 0,
        "hp_kp10": hp.get("killsPerTen") or 0,
        "snd_kd": snd.get("kdRatio") or 0,
        "snd_first_bloods": snd.get("firstBloods") or 0,
        "ovl_kp10": ctrl.get("killsPerTen") or 0,
        "maps_played": (css.get("aggregate") or {}).get("mapsPlayed") or 0,
    }
    g["has_usable_rates"] = bool(g["hp_kp10"] or g["snd_kd"] or g["ovl_kp10"] > 4)
    return g

def cito_grounding_block(team_a, team_b, rosters):
    """Build a text grounding block of real Cito stats to feed the AI."""
    lines = []
    for team in (team_a, team_b):
        for ign in (rosters.get(team) or []):
            g = cito_player_grounding(ign)
            if g:
                lines.append(f"  {ign} ({team}): K/D {g['kd']}, total kills {g['kills']}, "
                             f"HP kp10 {g['hp_kp10']}, SnD K/D {g['snd_kd']}, OVL kp10 {g['ovl_kp10']}, maps {g['maps_played']}")
...
def ai_stats_to_rows(team_a, team_b, ai_players, live_rosters=None):
    """Convert AI per-mode stat dicts to projected kills per mode."""
    lr = live_rosters or {}
    roster_a = lr.get(team_a) or get_roster_for(team_a)[0]
    roster_b = lr.get(team_b) or get_roster_for(team_b)[0]
    valid_names = {p.lower() for p in roster_a + roster_b}
    rows = []
    seen = set()
    for p in ai_players:
        ...
        # Trust the live roster: keep the player if the AI listed them on either team
        if not name: continue
        if team not in (team_a, team_b):
            team = team_a if name.lower() in {x.lower() for x in roster_a} else (team_b if name.lower() in {x.lower() for x in roster_b} else team)
    ...
    # Fill any current-roster players the AI missed (uses live roster, not baked-in)
    for team, roster in ((team_a, roster_a), (team_b, roster_b)):
        for player in roster:
            if (team, player) not in seen and player.lower() not in {n.lower() for (t, n) in seen if t == team}:
...
def render_analysis(parsed, polymarket, maps, stats_df):
    ...
    st.markdown("### Map-by-map breakdown")
    map_blocks = parsed.get("maps", [])
    if map_blocks:
        for m in map_blocks:
            mw = safe(m.get("map_winner"))
            mwp = to_num(m.get("map_winner_prob"))
            mwp_pct = round(mwp * 100) if mwp <= 1 else round(mwp)
            players = m.get("players", [])
            # sort by projected kills desc just in case
            players = sorted(players, key=lambda x: -to_num(x.get("projected_kills")))
            st.markdown(f"""<div class="match-card">
              <div class="muted">Map {m.get('map')} · {safe(m.get('mode'))} · {safe(m.get('map_name')) or 'TBD'}</div>
              <h3 style="margin:6px 0;">Map winner lean: <span class="good">{mw or '—'}</span>
                <span class="pill">{mwp_pct}%</span></h3>
              <p class="muted" style="margin:4px 0 0;font-size:13px;">{safe(m.get('map_reason'))}</p>
            </div>""", unsafe_allow_html=True)
            if players:
                dfm = pd.DataFrame([{
                    "Rank": i + 1,
                    ...
                    "Proj kills": to_num(p.get("projected_kills")),
                    "Note": safe(p.get("note")),
                } for i, p in enumerate(players)])
                st.dataframe(dfm, use_container_width=True, hide_index=True)
    else:
        # fallback to our own per-mode model if the AI didn't return map blocks
        st.info("AI didn't return per-map blocks — showing model projections by mode.")
        for m in build_targets(stats_df, maps):
            st.markdown(f"**Map {m['map']} · {m['mode']} · {m['map_name'] or 'TBD'}**")
            dfm = pd.DataFrame([{
                "Rank": i + 1, "Player": r["Player"], "Team": r["Team"], "Proj kills": r["ProjKills"], "Form": r.get("Form", "")
...
with st.sidebar:
    ...
    st.write("Cito:", "✅" if get_secret("CITO_API_KEY") else "❌ missing (optional)")
    model = st.text_input("OpenAI model", value="gpt-4.1-mini")
    use_cito = st.toggle("Use Cito data", value=bool(get_secret("CITO_API_KEY")),
                         help="ON: schedule + rosters come from Cito (most reliable), and Cito stats ground the AI. "
                              "OFF (or out of Cito credits): falls back to OpenAI + Breaking Point. Never blocks.")
...
with tab_match:
    ...
    c1.markdown('<div class="card">Pulls live + upcoming matches. With Cito on, the schedule comes straight from Cito (most reliable); otherwise OpenAI + Breaking Point.</div>', unsafe_allow_html=True)
    with c2:
        if st.button("🔄 Refresh", use_container_width=True, disabled=not (openai_key or get_secret("CITO_API_KEY"))):
            rows = []
            with st.spinner("Loading matches…"):
                cito_err = None
                if use_cito:
                    crows, cito_err = cito_schedule()
                    rows += crows
                # Only fall back to BP/AI if Cito gave nothing
                if not rows:
                    rows += bp_match_list()
                    if openai_key:
                        try:
                ...
                if cito_err and use_cito and rows:
                    pass  # had Cito rows, ignore minor errors
                elif cito_err and use_cito:
                    st.warning(f"Cito schedule unavailable ({cito_err}) — used fallback sources.")
            # dedupe
            seen, dd = set(), []
            for m in rows:
    ...
    if not st.session_state.matches:
        ...
        st.caption("As vetoes drop on Twitter/X, fill these in. **Mode** is fixed by the series format. "
                   "**Map** is the actual map being played (pick once the veto is known). "
                   "**Picked by** = which team chose that map in the veto (optional — it nudges that team's players up slightly, since teams pick maps they're strong on). Leave it blank if you don't know.")
        hc1, hc2, hc3, hc4 = st.columns([0.4, 1.3, 1.6, 1.4])
        hc1.markdown("**#**")
        hc2.markdown("**Mode**")
        hc3.markdown("**Map**")
        hc4.markdown("**Picked by** *(optional)*")
        new_maps = []
        for i, row in enumerate(st.session_state.maps):
            ...
            with cc4:
                picks = ["", match["team_a"], match["team_b"]]
        ...
        if go:
            try:
                sources = {"rosters": "AI", "schedule": match.get("source", "?"), "stats": "AI/BP", "odds": "—"}
                cito_ground = ""
                # 1. Rosters: Cito first (authoritative), AI fallback
                live_rosters = {}
                if use_cito:
                    with st.spinner("Loading rosters from Cito…"):
                        for team in (match["team_a"], match["team_b"]):
                            cr, cerr = cito_roster(team)
                            if cr:
                                live_rosters[team] = cr
                        if len(live_rosters) == 2:
                            sources["rosters"] = "Cito"
                if len(live_rosters) < 2:
                    with st.spinner("Confirming rosters via AI…"):
                        ai_r, _ = ai_confirm_rosters(openai_key, model, match["team_a"], match["team_b"])
                        for team in (match["team_a"], match["team_b"]):
                            live_rosters.setdefault(team, ai_r.get(team) or get_roster_for(team)[0])
                # 2. Cito stat grounding (real numbers where they exist)
                if use_cito:
                    with st.spinner("Pulling Cito stat grounding…"):
                        cito_ground = cito_grounding_block(match["team_a"], match["team_b"], live_rosters)
                        if cito_ground:
                            sources["stats"] = "Cito-grounded + AI/BP"
                # 3. Stats projection (AI/BP — Cito rate stats are unreliable)
                with st.spinner("Gathering player stats…"):
                    ai_players, raw_stats = ai_player_stats(openai_key, model, match["team_a"], match["team_b"], live_rosters)
                    stats_df = ai_stats_to_rows(match["team_a"], match["team_b"], ai_players, live_rosters)
                    pm = polymarket_cdl(match["team_a"], match["team_b"])
                    if pm: sources["odds"] = "Polymarket"
                # 4. Full analysis (Cito grounding appended to prompt)
                with st.spinner("Running full analysis…"):
                    prompt = ai_full_analysis(openai_key, model, match, new_maps, stats_df, live_rosters)
                    if cito_ground:
                        prompt += f"\n\nReal Cito stat grounding (use as a sanity check; some rate fields may be 0 if unsynced):\n{cito_ground}"
                    parsed = extract_json(openai_search(openai_key, model, prompt))
                st.session_state.saved[key] = {"saved_at": now(), "match": match, "maps": new_maps,
                                               "parsed": parsed, "polymarket": pm,
                                               "live_rosters": live_rosters, "sources": sources,
                ...
                if "insufficient_quota" in msg or "429" in msg:
                    st.error("OpenAI says your credit balance is empty. Add credit at platform.openai.com → Billing (a few £ lasts months), wait ~2 min, then Analyse again.")
                else:
        ...
        if active:
            ...
            srcs = active.get("sources")
            if srcs:
                badges = " ".join([f'<span class="pill">{k.capitalize()}: {v}</span>' for k, v in srcs.items()])
